System.register(['test/lib/common', '../dashboard_srv'], function(exports_1) {
    ...
    return {
        ...
        execute: function() {
            common_1.describe('dashboardSrv', function () {
                ...
                common_1.describe('when creating new dashboard with defaults only', function () {
                    var model;
                    common_1.beforeEach(function () {
                ...
                common_1.describe('row and panel manipulation', function () {
                    ...
                    common_1.it('row span should sum spans', function () {
                        var spanLeft = dashboard.rowSpan({ panels: [{ span: 2 }, { span: 3 }] });
                        common_1.expect(spanLeft).to.be(5);
                    });
                    common_1.it('adding default should split span in half', function () {
                        ...
                        common_1.expect(dashboard.rows[0].panels[1].id).to.be(8);
                    });
                ...
                common_1.describe('when creating dashboard with editable false', function () {
                    var model;
                    common_1.beforeEach(function () {
                        model = _dashboardSrv.create({
                            editable: false
                    ...
                    common_1.it('should set editable false', function () {
                        common_1.expect(model.editable).to.be(false);
                    });
                });
                common_1.describe('when creating dashboard with old schema', function () {
                    ...
                    common_1.it('dashboard schema version should be set to latest', function () {
                        common_1.expect(model.schemaVersion).to.be(13);
                    });
                    common_1.it('graph thresholds should be migrated', function () {
                        common_1.expect(graph.thresholds.length).to.be(2);
                        common_1.expect(graph.thresholds[0].op).to.be('>');
                        common_1.expect(graph.thresholds[0].value).to.be(400);
                        common_1.expect(graph.thresholds[0].fillColor).to.be('red');
                        common_1.expect(graph.thresholds[1].value).to.be(200);
                        common_1.expect(graph.thresholds[1].fillColor).to.be('yellow');
                    });
                });
